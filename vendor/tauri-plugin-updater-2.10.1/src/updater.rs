...
///
/// Manifests are normally only a few KiB and updater artifacts are expected to
/// stay well below 256 MiB. Keeping explicit defaults prevents any plugin entry
/// point from silently reverting to an unbounded in-memory response.
pub const DEFAULT_MAX_MANIFEST_SIZE: u64 = 1024 * 1024;
...
mod response_limit_tests {
    ...
    #[test]
    fn updater_builder_defaults_to_nonzero_response_limits() {
        let app = tauri::test::mock_app();
        let builder = UpdaterBuilder::new(
            app.handle(),
            Config {
                endpoints: vec![url::Url::parse("https://example.test/latest.json").unwrap()],
                ..Default::default()
            },
        );

        assert_eq!(builder.max_manifest_size, DEFAULT_MAX_MANIFEST_SIZE);
    ...
    #[test]
    fn zero_limit_rejects_first_nonempty_response() {
        install_test_crypto_provider();
        let (url, server) = serve_once(vec![
            b"HTTP/1.1 200 OK\r\nContent-Length: 1\r\nConnection: close\r\n\r\n",
            b"x",
        ]);
        let error = tauri::async_runtime::block_on(async move {
            let response = reqwest::Client::new().get(url).send().await.unwrap();
            read_response_limited(response, 0, "test response", |_, _| {})
                .await
                .unwrap_err()
        });
        server.join().unwrap();
    ...
    fn rejects_oversized_content_length_before_reading_body() {
        ...
        let error = tauri::async_runtime::block_on(async move {
            let response = reqwest::Client::new().get(url).send().await.unwrap();
            read_response_limited(response, 8, "test response", |_, _| {})
                .await
                .unwrap_err()
        });
        server.join().unwrap();
    ...
    #[test]
    fn rejects_chunked_response_without_content_length_at_stream_limit() {
        install_test_crypto_provider();
        let (url, server) = serve_once(vec![
        ...
        let error = tauri::async_runtime::block_on(async move {
            let response = reqwest::Client::new().get(url).send().await.unwrap();
            read_response_limited(response, 8, "test response", |_, total| {
...
impl UpdaterBuilder {
    ...
    /// Sets the maximum number of manifest response bytes retained in memory.
    /// Both `Content-Length` and the streamed byte count are enforced.
    pub fn max_manifest_size(mut self, max_bytes: u64) -> Self {
        self.max_manifest_size = max_bytes;
        self
    }

    /// Sets the maximum number of updater artifact bytes retained in memory.
    /// Both `Content-Length` and the streamed byte count are enforced.
    pub fn max_download_size(mut self, max_bytes: u64) -> Self {
        self.max_download_size = max_bytes;
        self
    }
...
impl Updater {
    pub async fn check(&self) -> Result<Option<Update>> {
            ...
            match response {
                Ok(res) => {
                    if res.status().is_success() {
                        ...
                        let manifest = read_response_limited(
                            res,
        ...
        let installer = installer_for_bundle_type(bundle_type());
        let (download_url, signature, platform) = self.get_urls(&release, &installer)?;
    ...
    fn get_urls<'a>(
        ...
        for target in &targets {
            ...
            {
                return Ok((download_url, signature, target.clone()));
            };
        }
...
pub struct Update {
    ...
    /// this is either the user provided target or the current operating system by default
    pub target: String,
    /// The exact platform key selected from a static release manifest.
    pub platform: String,
...
impl Update {
    ...
    pub async fn download<C: FnMut(usize, Option<u64>), D: FnOnce()>(
        ...
        let buffer = read_response_limited(
            response,
...
async fn read_response_limited<C>(
...
// Validate signature
/// Verifies bytes with the same minisign format used for updater artifacts.
///
/// This is public so applications can authenticate auxiliary release identity
/// files with the updater key before trusting an unsigned mirror manifest.
